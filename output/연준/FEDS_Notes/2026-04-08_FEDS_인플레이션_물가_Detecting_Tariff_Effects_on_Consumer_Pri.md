# FEDS Notes - Detecting Tariff Effects on Consumer Prices in Real Time â Part II
...
지난해 미국 무역 정책의 큰 변화로 인해 관세의 경제적 효과에 대한 시기적절한 평가에 대한 관심이 급증했습니다. Minton과 Somale(2025)은 공개적으로 이용 가능한 데이터에만 의존하여 소비자 가격에 대한 관세 효과를 실시간으로 탐지하는 방법론을 개발했습니다. 이 접근 방식에 따라 저자는 먼저 시행된 관세 변경, 각 범주에서 관세가 부과된 수입품의 보급, 관세에서 가격으로의 전체 달러 전가 가정을 기반으로 개별 개인 소비 지출(PCE) 범주의 가격에 관세가 미치는 영향에 대한 이론적 예측을 구성한 다음 이러한 이론적 효과가 관찰된 PCE 가격 변화와 일치하는지 평가합니다. Minton과 Somale(2025)은 작년 초에 시행된 미국의 중국 관세가 이미 3월까지 소비자 가격에 영향을 미치고 있다는 예비 증거를 발견했습니다.
...
그런 다음 이 세부 관세 정보를 집계하여 BEA의 GVC IO 테이블 집계 수준에서 관세율을 계산합니다. 구체적으로, 우리는 먼저 일치 테이블을 사용하여 세부 관세 데이터 세트의 각 HS10 수출업체 쌍을 BEA GVC IO 테이블의 상품-원산지 쌍에 할당합니다. 그런 다음 할당된 HS10-수출자 쌍의 관세율의 가중 평균으로 후자 각각에 대한 관세율을 계산하며 가중치는 2024년 수입 점유율로 지정됩니다.
...
Minton and Somale(2025)에서 더 자세히 논의된 것처럼, 데이터에서 실현된 관세 효과가 이론적 관세 효과와 다를 수 있는 많은 이유가 있습니다. 예를 들어, 이론적 측정에서 가정되는 달러당 전체 이월은 일정한 백분율 마진 하에서 전체 이월보다 항상 낮다는 점을 상기하십시오. 결과적으로 Minton과 Somale(2025)이 발견한 것처럼 추정된 통과율은 전체 달러 대비 통과율이 암시하는 것보다 높을 수 있습니다. 그러나 중요한 점은 아래의 분석에서 이론적 측정에 의해 예측된 대로 가격이 상승했는지 여부를 실증적으로 평가할 것이며, 실증적 분석의 어떤 요소도 이를 예상하는 이론적 이유에도 불구하고 통과가 발생할 것을 요구하지 않습니다.
...
참고: 패널(a)에서 통과 계수는 누적됩니다. 값 1은 관세가 상대 소비자 가격으로 완전히 전가되는 것을 의미합니다. 회귀 표본 기간은 2024년 1월부터 2026년 2월까지이며 관측값에는 가중치가 적용되지 않습니다. 90% 신뢰 구간이 표시되고 표준 오류는 PCE 범주별로 클러스터링됩니다. 패널 (b)에서 파란색으로 표시된 예측 지역은 표 1의 범위 외에 추가 관세 변경이 없으며 관세 변경 후 7개월 후에 추가 관세 통과가 없다고 가정합니다.
...
그림 A1은 9개의 시차로 추정된 모델에서 9개월의 누적 관세 이전이 약 100%로 유지됨을 보여줍니다. 실제로 이는 기준선 그림 3에 표시된 7개월 누적 관세 통과와 유사한 수준으로 유지됩니다. 그림 A2는 5개의 시차로 추정된 모델에서 5개월 누적 관세 통과가 거의 100%에 가까운 것으로 나타남을 보여줍니다. 신뢰 구간이 넓기 때문에 합리적이지만 여전히 잠정적인 결론은 관세 통과가 약 100% 안정화되고 발생하는 데 5~9개월이 걸린다는 것입니다.
...
4. 공식 커뮤니케이션은 이 조치의 영향을 받는 제품 및 원산지 국가뿐만 아니라 영향을 받는 수입품에 대한 관세율에 대한 정보를 제공합니다. 이 정보는 관세가 해당 쌍 내의 모든 수입품에 적용될 때 HS10-수출자 쌍에 의한 관세율의 변화를 쉽게 산출합니다.  텍스트로 돌아가기

5. Gopinath 및 Neiman(2026) 및 Besten 및 Kanzig(2026)는 이러한 제한 중 일부를 완화하기 위해 2024년 무역 가중치를 사용하여 평균 관세율을 계산하는 등 세입 측정을 사용하지만, 이러한 접근 방식 중 어느 것도 관세가 높은 상품의 수입이 0으로 떨어질 때 발생하는 하향 편향을 완전히 해결할 수 없습니다.  텍스트로 돌아가기
...
7. Minton과 Somale(2025)은 일반 균형 효과가 나타나는 데 더 오랜 시간이 걸리기 때문에 2025년 2월~3월에 추정한 "1차" 관세 효과가 당시의 총 관세 효과와 동일할 가능성이 높다고 주장했습니다. 2025년 최초 관세가 시행된 이후 충분한 시간이 흘렀기 때문에 이제는 '2차' 효과가 더 의미 있을 수 있다고 생각합니다.  텍스트로 돌아가기
...
10. 관세는 핵심 서비스 소비 비용보다 핵심 상품 소비 비용에 훨씬 더 큰 영향을 미칩니다. 자동차 수리 서비스와 같은 일부 서비스는 관세로 인해 상당히 높은 비용을 경험할 수 있지만, 이 평가(프레임워크를 사용하여 가능)는 향후 연구로 남겨둡니다.  텍스트로 돌아가기
...
14. 회귀 분석에 가중치가 적용되지 않는다고 언급했지만 PCE에 의한 관측값에 가중치를 적용하면 결과가 의미 있게 다르지 않습니다.  텍스트로 돌아가기
...
30. 표준관행에서는 기업이 USMCA를 준수하기로 한 결정과 같은 내생변수에 대해 관세 전 정보를 사용하는 것을 선호합니다. 왜냐하면 이러한 변수 자체가 관세의 영향을 받을 수 있기 때문입니다.  텍스트로 돌아가기
...
34. 주요 분석에 사용하는 BEA의 2023년 GVC IO 테이블보다 산업 세부정보가 더 많은 BEA의 2017년 벤치마크 미국 입출력 테이블이 금속 함량을 계산하는 데 더 적합합니다. 그러나 이러한 벤치마크 테이블에는 분석의 핵심 입력인 원산지별로 분류된 가져오기가 없습니다.  텍스트로 돌아가기
...
1. When studying the 2018-19 U.S. tariffs on China, Minton and Somale (2025) abstract from tariff variation across products by considering instead a flat tariff equal to the average rate on all US imports from China implied by the policy changes under consideration.  Return to text
