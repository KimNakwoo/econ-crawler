# FEDS Notes - AI Adoption and Firms' Job-Posting Behavior
...
이 노트에서는 두 가지 데이터 소스와 두 가지 서로 다른 변형 소스를 사용하여 AI 채택과 기업의 채용 공고 행동 사이의 관계를 조사합니다. 채용 공고는 65,000개 이상의 소스에서 채용 공고를 카탈로그화한 Lightcast(이전의 Burning Glass) 데이터베이스에서 가져옵니다. 3 이 데이터베이스는 기능적으로 온라인에 게시된 다양한 채용 정보를 포함합니다. 온라인에 게시된 일자리의 구성이 연도마다 다를 가능성이 높기 때문에 장기간 비교가 어려워지지만, 2022년 이후 데이터만 사용하는 현재 연구에서는 이는 문제가 되지 않습니다. 더욱이, 이 데이터는 직업/산업 대표성을 살펴볼 때 다른 출처를 대표하는 것으로 밝혀졌습니다(Hershbein and Kahn 2019).
...
$$$$y_{i,t}=\alpha+\ \beta{입양}_{i,t-k}+\gamma X_i+{연도}_t+\ \varepsilon_{i,t}$$$$
...
참고: 각 셀은 식 (1)의 AI 채택 처리 변수에 대한 회귀 계수를 나타냅니다. 회사 수준에서 클러스터링된 표준 오류는 각 계수 아래에 표시되며 통계적 유의성은 .10, .05 및 .01 수준에서 각각 *, ** 및 ***로 표시됩니다.
...
이 모델 세트는 이질적인 효과를 조사하기 위해 세 가지 다른 샘플에서 실행됩니다. 모든 회사, AI/ML 채용 공고가 있는 회사만, 대규모 회사(2023~2025 샘플 기간 동안 최소 300개의 채용 공고가 있는 것으로 정의됨). 기업 수준의 AI 투자가 후속 채용 공고 행동에 부정적인 영향을 미친다는 증거는 다양한 모델 전반에 걸쳐 존재하지 않습니다.

산업 노출 모델과 마찬가지로 기업 노출 계수의 크기는 치료 변수의 평균 정도에 따라 적절하게 조정될 때 작습니다. Lightcast 데이터의 크기가 크기 때문에 통계적 유의성은 주어진 치료의 중요성을 나타내는 유용한 척도가 아니라는 점을 기억하는 것이 중요합니다. 전체 기업에서 AI 관련 게시물의 비중은 1.6%에 불과하고, AI 관련 게시물을 올린 적이 있는 기업은 8.6%, 대기업은 2.5%에 불과하다. 즉, 계수의 맨 윗줄에 대한 완전히 인과적인 해석(반복하자면, 우리의 주장은 아님)은 AI 채택으로 인해 2025년 채용 공고가 0.04%에서 0.13% 사이로 증가했음을 의미합니다. 따라서 우리는 표 2의 계수를 정확하게 추정된 영 효과로 봅니다.
...
이는 인과관계가 아닐 가능성이 높으며, AI에 투자하는 산업이나 기업의 유형과 관련된 요소를 보다 그럴듯하게 반영한다는 점에 유의하는 것이 중요합니다. 대신, 우리는 우리의 결과를 지금까지 AI로 인해 노동 시장이 악화되었다는 이야기에 대한 단순한 증거로 봅니다. 더욱이, 이 활동은 명시적으로 과거 지향적이며 AI와 노동 시장의 향후 관계에 대해 어떠한 암시도 하지 않습니다. 하지만 우리는 이를 지속적으로 모니터링할 계획입니다.
...
Brynjolfsson, Erik, Danielle Li, Lindsey Raymond. “직장에서의 생성적 AI.”  분기경제학저널 140, no. 2(2025년 5월): 889-942. https://doi.org/10.1093/qje/qjae044.
...
니본, 엘리자베스, 나탈리 홈즈. " 저소득 근로자의 AI 현장 노출 ." 샌프란시스코 연방준비은행 지역사회 개발 연구 개요 2025-03. 2025년 11월 21일.
...
4. 모든 모델은 원시 게시물 수가 아닌 채용 게시물의 자연 로그를 종속 변수로 사용하여 실행되었습니다. 결과는 종속변수의 기능적 형태에 관계없이 질적으로 동일합니다.  텍스트로 돌아가기
...
---
*출처: https://www.federalreserve.gov/econres/notes/feds-notes/ai-adoption-and-firms-job-posting-behavior-20260327.html | 수집: 2026-06-10 01:29 | 지표: FEDS_NOTES*
